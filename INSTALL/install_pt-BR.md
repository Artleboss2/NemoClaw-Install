...
O NemoClaw é uma stack de código aberto da NVIDIA projetada para executar agentes de IA OpenClaw com segurança, adicionando controles de privacidade, sandboxing (ambiente de testes isolado) e barreiras de segurança.
...
## Passo 1: Preparar o seu sistema

O NemoClaw exige a instalação do Docker e do Node.js (versão 20 ou 22) para funcionar corretamente.
...
(Se você receber um erro de permissão negada "permission denied", execute `sudo usermod -aG docker $USER`, saia da conta e faça login novamente).
...
Observação: Dependendo da configuração do seu sistema, pode ser necessário executá-lo com `sudo bash` se você encontrar erros de permissão.

## Passo 3: Configurar o seu agente (Onboarding)
...
Observação: O script fará o download da imagem do sandbox (aprox. 2,4 GB). Isso pode levar alguns minutos.

## Passo 4: Conectar e conversar!

Seu ambiente seguro agora está pronto. É hora de se conectar ao seu agente isolado.

### 1. Conectar-se ao sandbox:
